...
export const createDoctor = async (doctorData: { user_id: number; specialty: string; crm:string; accept_money: boolean; accept_plan: boolean }) => {
  try {
...
export const updateDoctor = async (doctorId: number, updatedData: { user_id?: number; specialty?: string; crm?:string ;accept_money?: boolean; accept_plan?: boolean }) => {
  try {
    const doctor = await Doctor.findByPk(doctorId);
    if (!doctor) {
      throw new Error('Doctor not found');
